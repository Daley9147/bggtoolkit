-- Recreate the function with corrected JSONB parameter types
create or replace function public.create_company_and_outreach_plan(
    ...
    p_insights text,
    p_email text,
    p_email_subject_lines jsonb, -- Changed from text[]
    p_linkedin_connection_note text,
    p_linkedin_follow_up_dm text,
    p_cold_call_script text,
    p_follow_up_email_subject_lines jsonb, -- Changed from text[]
