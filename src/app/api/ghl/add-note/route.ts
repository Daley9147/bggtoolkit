...
async function ghlRequest(url: string, accessToken: string, locationId: string, options: RequestInit = {}) {
...
export async function POST(request: Request) {
  ...
  if (!contactId || !note) {
    return NextResponse.json({ error: 'Contact ID and note are required.' }, { status: 400 });
  }
  ...
  try {
    ...
    console.error('Error adding note to GHL contact:', error);
