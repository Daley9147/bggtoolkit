...
export async function GET(request: Request) {
  ...
  try {
    const url = new URL('https://services.leadconnectorhq.com/calendars/blocked-slots');
    url.searchParams.append('locationId', profile.ghl_location_id!);
    url.searchParams.append('userId', profile.ghl_user_id!);
    url.searchParams.append('startTime', new Date(startDate).getTime().toString());
    url.searchParams.append('endTime', new Date(endDate).getTime().toString());
    ...
    if (!response.ok) {
      ...
      console.error('GHL Blocked Slots API Error:', errorData);
      return NextResponse.json({ error: 'Failed to fetch blocked slots from GHL.', details: errorData }, { status: response.status });
    }
    ...
    console.error('Error fetching GHL blocked slots:', error);
    return NextResponse.json({ error: 'An unexpected error occurred while fetching blocked slots.' }, { status: 500 });
  }
}
